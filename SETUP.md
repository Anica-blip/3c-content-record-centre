...
- Opening a card for edit or view must always clone the record, never hand it a direct reference to the in-memory list entry. Editing a field mutates whatever object the card holds — if that's the *same* object sitting in the live list, the screen shows the new value instantly, looking saved, even if the actual save to storage never happened (or failed). This exact bug produced a confusing symptom: the card looked correct, the list looked correct, but a CSV export (which re-fetches from the server first) showed stale data — because the export was the only thing actually checking the true stored state. `openCardFlow()` now does `JSON.parse(JSON.stringify(record))` before handing it to a card, specifically to prevent this.
